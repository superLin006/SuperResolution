...
int main(int argc, char **argv)
{
    if (argc != 3)
    {
        ...
        printf("Example: %s model/edsr_x4_fp.rknn model/test_input_256x256.png\n", argv[0]);
        return -1;
    }
    ...
    int ret;
    rknn_edsr_context_t edsr_ctx;
    memset(&edsr_ctx, 0, sizeof(rknn_edsr_context_t));

    // Initialize model
    printf("========================================\n");
    printf("EDSR Super-Resolution Demo\n");
    printf("========================================\n");
    ret = init_edsr_model(model_path, &edsr_ctx);
    if (ret != 0)
    {
        printf("init_edsr_model fail! ret=%d model_path=%s\n", ret, model_path);
        return -1;
    }
    ...
    ret = inference_edsr_model(&edsr_ctx, &src_image, &dst_image);

    gettimeofday(&stop_time, NULL);
    if (ret != 0)
    {
        printf("inference_edsr_model fail! ret=%d\n", ret);
        goto out;
    }
...
out:
    ...
    ret = release_edsr_model(&edsr_ctx);
    if (ret != 0)
    {
        printf("release_edsr_model fail! ret=%d\n", ret);
    }
