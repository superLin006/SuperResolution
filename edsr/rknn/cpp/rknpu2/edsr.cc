...
int init_edsr_model(const char *model_path, rknn_edsr_context_t *app_ctx)
{
    ...
    printf("Loading EDSR RKNN model...\n");
...
int inference_edsr_model(rknn_edsr_context_t *app_ctx, image_buffer_t *src_img, image_buffer_t *dst_img)
{
    int ret;
    int hw;
    ...
    if (src_img->width != app_ctx->model_width || src_img->height != app_ctx->model_height)
    {
        ...
        // Resize using RGA
        // Note: convert_image signature: (src, dst, src_box, dst_box, bg_color)
        // For simple resize, we can use convert_image_with_letterbox or manual resize
        // For now, we'll just use the input directly with warning
        printf("WARNING: Input size mismatch, using input directly (may cause issues)\n");
        // TODO: Implement proper image resize
    ...
    // Debug: Print first few pixels of input (RGB order)
    printf("Input pixels (HWC RGB format):\n");
    printf("  [0,0]: R=%d, G=%d, B=%d\n",
           input_img.virt_addr[0], input_img.virt_addr[1], input_img.virt_addr[2]);
    printf("  [0,1]: R=%d, G=%d, B=%d\n",
           input_img.virt_addr[3], input_img.virt_addr[4], input_img.virt_addr[5]);
    printf("  [1,0]: R=%d, G=%d, B=%d\n",
           input_img.virt_addr[app_ctx->model_width*3],
           input_img.virt_addr[app_ctx->model_width*3+1],
           input_img.virt_addr[app_ctx->model_width*3+2]);

    // Set Input Data - NHWC format as expected by RKNN model
    // The model expects NHWC FP16 input, RKNN will convert uint8 to FP16
    inputs[0].index = 0;
    inputs[0].type = RKNN_TENSOR_UINT8;
    inputs[0].fmt = RKNN_TENSOR_NHWC;
    inputs[0].size = input_img.size;
    inputs[0].buf = input_img.virt_addr;
    inputs[0].pass_through = 0;  // Let RKNN handle conversion to FP16
    ...
    // Run inference
    printf("Running super-resolution inference...\n");
    ...
    // Get Output - For INT8 models, we want RKNN to convert to float for us
    outputs[0].index = 0;
    if (app_ctx->is_quant)
    {
    ...
    // Debug: Print output buffer info
    printf("Output buffer info:\n");
    printf("  buf=%p, size=%u, want_float=%d, is_prealloc=%d\n",
           outputs[0].buf, outputs[0].size, outputs[0].want_float, outputs[0].is_prealloc);

    // Post Process: Convert float output to RGB888
    ...
    if (app_ctx->is_quant)
    {
        ...
        // Debug: Print channel values at pixel [0,0]
        printf("Debug - Output at pixel [0,0] (from dequantized float):\n");
        printf("  ch0[0] = %.2f (index 0)\n", float_data[0]);
        printf("  ch1[0] = %.2f (index %d)\n", float_data[hw], hw);
        printf("  ch2[0] = %.2f (index %d)\n", float_data[2*hw], 2*hw);
    ...
    else
    {
        ...
        // Debug: Print channel values at pixel [0,0] (convert FP16 to float for display)
        printf("Debug - Output at pixel [0,0] (from FP16):\n");
        printf("  ch0[0] = %.2f (index 0)\n", fp16_to_float(fp16_data[0]));
        printf("  ch1[0] = %.2f (index %d)\n", fp16_to_float(fp16_data[hw]), hw);
        printf("  ch2[0] = %.2f (index %d)\n", fp16_to_float(fp16_data[2*hw]), 2*hw);
