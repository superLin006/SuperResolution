#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Convert ONNX EDSR model to RKNN format."""
...
def create_dataset_file():
    ...
    for fname in ['input_256x256.png', 'test_input.png']:
...
def parse_arg():
    ...
    if len(sys.argv) < 3:
        ...
        print("\nExamples:")
        print("  python convert.py ../model/edsr_x4.onnx rk3576")
        print("  python convert.py ../model/edsr_x4.onnx rk3576 fp ../model/edsr_x4_fp16.rknn")
        print("  python convert.py ../model/edsr_x4.onnx rk3576 i8 ../model/edsr_x4_int8.rknn")
        print("\nNote: FP16 ('fp') is recommended for super-resolution to maintain quality.")
        print("      INT8 ('i8') is faster and may fix color issues in some cases.")
        exit(1)
    ...
    if len(sys.argv) > 4:
        ...
        dtype_suffix = '_int8' if do_quant else '_fp16'
...
def main():
    ...
    # Load ONNX model
    # EDSR typically uses 256x256 input (can be adjusted)
    # Check input size from ONNX model first
    print(f'\n--> Loading ONNX model')
    input_size_list = [[1, 3, 256, 256]]  # NCHW format
