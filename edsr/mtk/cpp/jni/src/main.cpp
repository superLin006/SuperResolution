/**
 * EDSR Super-Resolution - Main Program
 * Usage: edsr_inference <model_path> <input_image> <output_image>
 */
...
int main(int argc, char** argv) {
    if (argc != 4) {
        ...
        std::cout << "Example: " << argv[0] << " EDSR_x4_256x256_MT8371.dla input.png output.png" << std::endl;
        return -1;
    }
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "EDSR Super-Resolution (MTK NPU)" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    // Initialize EDSR
    edsr_context_t edsr_ctx;
    memset(&edsr_ctx, 0, sizeof(edsr_ctx));

    auto start_init = std::chrono::high_resolution_clock::now();
    ret = edsr_init(model_path, &edsr_ctx);
    auto end_init = std::chrono::high_resolution_clock::now();

    if (ret != 0) {
        std::cerr << "Failed to initialize EDSR model!" << std::endl;
        return -1;
    }
    ...
    ret = edsr_inference_ex(&edsr_ctx, input_image, width, height,
    ...
    std::cout << "  Scale:    " << edsr_ctx.scale_factor << "x" << std::endl;
