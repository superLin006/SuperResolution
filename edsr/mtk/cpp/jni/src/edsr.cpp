...
int edsr_init(const char* model_path, edsr_context_t* ctx) {
    ...
    memset(ctx, 0, sizeof(edsr_context_t));

    std::cout << "[INFO] Initializing EDSR..." << std::endl;
    ...
    std::unique_ptr<NeuronExecutor> executor(
        new NeuronExecutor(model_path, input_shapes, output_shapes, "EDSR"));
    ...
    std::cout << "[INFO] EDSR initialized successfully!" << std::endl;
...
int edsr_release(edsr_context_t* ctx) {
    ...
    std::cout << "[INFO] Releasing EDSR resources..." << std::endl;
...
int edsr_inference(edsr_context_t* ctx,
                   ...
                   unsigned char** output_rgb) {
    return edsr_inference_ex(ctx, input_rgb, ctx->input_width, ctx->input_height,
...
int edsr_inference_ex(edsr_context_t* ctx,
    ...
    std::cout << "[INFO] Running EDSR inference..." << std::endl;
